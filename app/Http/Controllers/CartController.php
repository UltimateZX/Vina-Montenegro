...
use Illuminate\Http\JsonResponse; // <-- Importante para las respuestas JSON
...
class CartController extends Controller
{
    ...
    /**
     * Devuelve los contenidos del carrito en formato JSON.
     */
    public function index(): JsonResponse
    {
        $cart = session()->get('cart', []);

        // El frontend espera un array de objetos, no un objeto con IDs como claves.
        // Usamos array_values() para convertirlo al formato correcto.
        // También añadimos el 'id' a cada item, que es crucial para el frontend.
        $formattedCart = [];
        foreach ($cart as $id => $details) {
            $formattedCart[] = [
                'id' => $id,
                'name' => $details['nombre'], // Mapeamos a 'name' como espera el frontend
                'quantity' => $details['quantity'],
                'price' => $details['precio'], // Mapeamos a 'price'
                'image' => $details['url_imagen'] // Mapeamos a 'image'
            ];
        }
    ...
    /**
     * Añade un producto al carrito y devuelve el carrito actualizado.
     */
    public function add(Request $request): JsonResponse
    {
        ...
        return response()->json(['message' => 'Producto añadido con éxito.']);
    }
